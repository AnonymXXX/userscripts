...
- **站点匹配自动填充**：账号绑定 URL/域名后，登录页自动检测验证码输入框并填入
  ...
  - **自定义站点规则**：内网站点输入框不标准时，通过油猴菜单配置「域名 → 字段选择器 → 提交按钮」
  - 
- **悬浮面板**：仅在页面存在验证码输入框时显示（登录后自动隐藏）；实时验证码 + 倒计时，一键复制 / 填入 / 增删账号
- **备份导出**：一键导出全部账号为 otpauth URI 复制到剪贴板
- 内置 JumpServer 规则（`10.0.40.10`、`jumpserver.yndysc.com`），账号支持逗号分隔多域名
...
3. 给账号绑定站点：面板「添加」或编辑时填写站点 URL/域名（如 `10.0.40.10`），匹配后登录页自动填充
